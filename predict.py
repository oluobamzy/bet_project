...
def load_model(league_name=None):
    ...
    try:
        ...
        if feature_path.exists():
            ...
            logging.info(f"✅ Loaded feature list: {feature_list['features']}")
        ...
        logging.error(f"❌ Failed to load model for {model_key}: {e}")
...
try:
    ...
    raise FileNotFoundError("❌ No model file (.pkl) found in models/ folder! Please train a model first.")
...
def predict_bet(league: str = "EPL") -> str:
    ...
    try:
        ...
        logging.error(f"❌ Failed to load model for {league}: {e}")
        return f"❌ Error loading prediction model: {str(e)}"
...
def predict_bet_tomorrow(league_name="EPL") -> str:
    ...
    try:
        ...
        logging.error(f"❌ Failed to load model for {league_name}: {e}")
        return f"❌ Error loading prediction model: {str(e)}"
